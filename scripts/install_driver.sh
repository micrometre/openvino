...
# Update system
# Remove any stale Intel GPU repo that may reference an unsupported codename (e.g. resolute).
INTEL_GPU_LIST="/etc/apt/sources.list.d/intel-graphics.list"
if [ -f "$INTEL_GPU_LIST" ]; then
    echo -e "${YELLOW}Removing stale Intel GPU APT repo to prevent update errors.${NC}"
    sudo rm -f "$INTEL_GPU_LIST"
fi
...
if apt-cache search intel-level-zero | grep -q level-zero; then
    ...
    # Intel GPU repo lags behind Ubuntu releases; fall back to noble (24.04) for 26.04+
    INTEL_GPU_CODENAME="$UBUNTU_CODENAME"
    if [[ "$UBUNTU_CODENAME" == "resolute" ]] || ! wget -q --spider "https://repositories.intel.com/gpu/ubuntu/dists/${UBUNTU_CODENAME}/InRelease" 2>/dev/null; then
        echo -e "${YELLOW}Ubuntu '${UBUNTU_CODENAME}' not yet in Intel GPU repo, falling back to noble.${NC}"
        INTEL_GPU_CODENAME="noble"
    fi

    wget -qO - https://repositories.intel.com/gpu/intel-graphics.key | sudo gpg --dearmor --output /usr/share/keyrings/intel-graphics.gpg
    echo "deb [arch=amd64 signed-by=/usr/share/keyrings/intel-graphics.gpg] https://repositories.intel.com/gpu/ubuntu ${INTEL_GPU_CODENAME} client" | sudo tee /etc/apt/sources.list.d/intel-graphics.list
...
if command -v vulkaninfo &> /dev/null; then
    ...
    # Lunarg repo lags behind Ubuntu releases; fall back to noble (24.04) for 26.04+
    LUNARG_CODENAME="$UBUNTU_CODENAME"
    if [[ "$UBUNTU_CODENAME" == "resolute" ]] || ! wget -q --spider "https://packages.lunarg.com/vulkan/dists/${UBUNTU_CODENAME}/InRelease" 2>/dev/null; then
        echo -e "${YELLOW}Ubuntu '${UBUNTU_CODENAME}' not in Lunarg repo, falling back to noble.${NC}"
        LUNARG_CODENAME="noble"
    fi

    # Add repository source
    echo "deb [arch=amd64 signed-by=/usr/share/keyrings/lunarg-graphics.gpg] https://packages.lunarg.com/vulkan ${LUNARG_CODENAME} main" | \
        sudo tee /etc/apt/sources.list.d/lunarg-vulkan-${LUNARG_CODENAME}.list > /dev/null || {
